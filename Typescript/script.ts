var outBox:HTMLInputElement=document.querySelector('#screen');
var btn:NodeListOf<Element>=document.querySelectorAll('.btn');
var scndOpr:boolean=true;
...
function memoryFunctions(input:string){
    ...
    function memCal(value:number, index:number, array:number[]){
        res+=value;
    }
...
function trigoFunctions(input:string){
    if(degree){
...
function functions(input:string){
    if(input==='round'){
...
function evaluate(input:string):number{
    ...
    if(input!==''){
        var res:string= input.split('-(').join('-').split(')-').join('-')
...
btn.forEach(item=>{
    ...
    item.addEventListener('click',(e:Event)=>{
        let btnText = e.target as HTMLElement;
        let btntext:string=btnText.innerText;
        if(btntext == 'DEG' ||
           btntext == 'RAD')
        {
            deg();
